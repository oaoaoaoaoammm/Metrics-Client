...
namespace Metrics_app
{
    ...
    public partial class HddControl : UserControl, INotifyPropertyChanged
    {
        ...
        {
            HddMetrics hddMetrics = await metricsAgentClient.GetHddMetrics(new GetAllHddMetricsApiRequest()
            {
            ...
            try
            {
                if (hddMetrics != null)
                {
                    Task.Run(() =>
                    {
                        LastLecture = hddMetrics.Value;
                    });
                    _trend = Convert.ToDouble(hddMetrics.Value);
                    Id++;
                }
            }
            catch (Exception ex) { }
